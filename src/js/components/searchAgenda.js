						...
						({ store, actions }) => {
							...
							return (
								<div>
									...
									<select  id="agenda" name = "valueselect" value={store.valueselect}  className="form-control" onChange={(e) => {actions.updateValueselect(e.target.value); actions.passgetConcat();}}>
										<Context.Consumer>
											{
												({ store, actions }) => {
